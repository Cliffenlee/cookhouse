...
export default function Back({toHomePage, recipe, selector}) {
    const imageLink = recipe.image_name ? `https://cookhouse-images.s3-ap-southeast-1.amazonaws.com/${recipe.image_name}` : undefined
    ...
    return (
        <Box className="back">
            {imageLink ? <Image height="50%" maxWidth="100%" src={imageLink} alt={recipe.name}/> : <DefaultPicture/>}
                ...
                <Flex width="100%" height="20%" mt={2}  justifyContent="start" alignItems="center">
                    ...
                    <Flex mt={4} p={0} justifyContent="start" alignItems="center" overflowX="scroll" overflowY="hidden">
                        {recipe.tools.map((tool, index) => {
                            return (
                                <Flex key={index} p={0} alignItems="center">
                                    <Tag colorScheme="green" mx={2}>{tool.tool_name}</Tag>
                                </Flex>
                            )
                        })}
                    </Flex>
